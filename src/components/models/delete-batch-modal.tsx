...
import { deleteBatch } from "@/api/educationBatchApi";
...
export const DeleteBatchDialog = () => {
  ...
  const isModalOpen = isOpen && type === "deleteBatch";
      ...
      <DialogContent className="sm:max-w-[400px] gap-0 p-0 overflow-hidden border bg-background/95 backdrop-blur-xl shadow-2xl">
        <div className="p-6 space-y-4">
          ...
          <div className="p-4 rounded-xl bg-muted/30 border border-border/50 space-y-1">
            ...
            <p className="text-sm font-bold text-foreground">{batch?.name || "Selected Batch"}</p>
          </div>

          <p className="text-xs text-muted-foreground leading-relaxed">
            Archiving this batch will remove it from the active batch list and unenroll all students.
